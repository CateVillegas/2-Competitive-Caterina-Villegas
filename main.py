...
def run(cmd, label):
    ...
    if r.returncode != 0:
        print(f"⚠️  '{label}' terminó con código {r.returncode}")
...
def main():
    ...
    if not args.analysis_only:
        if args.real:
            ...
            if not ok:
                print("⚠️  Scraper falló → usando datos mock como fallback")
    ...
    run([sys.executable, "analysis/generate_analysis.py"], "Generando análisis y charts")
